...
{
    CRC32InputMessage::CRC32InputMessage(const CoderMessage& base):
    CoderMessage(base), input(base.underlying["input"].get<std::string>())
    {
    }

    CRC32OutputMessage::CRC32OutputMessage(const CoderMessage& base):
    CoderMessage(base), checksum(base.underlying["checksum"].get<size_t>())
    {

    }

    nlohmann::json CRC32OutputMessage::to_json() const
    {
        underlying["checksum"] = checksum;
        underlying["message_direction"] = MessageDirectionType::MT_OUTPUT;
        return CoderMessage::to_json();
    }
